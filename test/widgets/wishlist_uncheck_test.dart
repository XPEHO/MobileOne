...
void main() {
  ...
  testWidgets('The wishlist menu should find deletion widget',
    ...
    //Then
    expect(find.byKey(Key("restartWishlist")), findsOneWidget);

    //When
    await tester.tap(find.byKey(Key("restartWishlist")));
    await tester.pumpAndSettle(Duration(seconds: 2));

    //Then
    expect(find.byKey(Key("confirmWishlistRestart")), findsOneWidget);
    expect(find.byKey(Key("cancelWishlistRestart")), findsOneWidget);
  });
}
